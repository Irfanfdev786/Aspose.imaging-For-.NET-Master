...
namespace Aspose.Imaging.Live.Demos.UI.Config
{
    ...
    public class GlobalAppHelper
    {
		...
		public GlobalAppHelper(HttpContext hc, HttpApplicationState appState, string SessionID, string language)
		{
			...
			AsposeImagingContext.atcc = new AsposeImagingContext(hc); // sync the context
		...
		private void initResources(string ResourcesFile, string SessionID)
		{
			...
			if (AsposeImagingContext.Current.Cache[SessionID] == null)
			{
				...
				AsposeImagingContext.Current.Cache.Add(
				   SessionID,
					resources,
